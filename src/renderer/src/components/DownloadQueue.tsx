...
export default function DownloadQueue(): React.JSX.Element | null {
  ...
  return (
    <div className="panel absolute right-3 top-3 z-20 flex w-72 flex-col gap-2 rounded-xl p-3">
      <div className="flex items-center justify-between px-1">
        <p className="text-xs font-medium text-ink/50">下载队列</p>
        {hasFinished && (
          <button
            onClick={clearFinishedDownloads}
            className="text-xs text-ink/35 transition-colors hover:text-ink/70"
          >
            清空已完成
          </button>
        )}
      ...
      <ul className="flex max-h-72 flex-col gap-2 overflow-y-auto">
        {downloads.map((d) => (
          <li key={d.taskId} className="panel-sm rounded-lg px-2.5 py-2">
            <div className="flex items-center gap-2">
              ...
              {d.status === 'canceled' && (
                <SlashIcon className="size-3.5 shrink-0 text-ink/35" />
              )}
              ...
              {d.status === 'downloading' && (
                <button
                  ...
                  title="取消下载"
                  className="shrink-0 rounded p-0.5 text-ink/35 transition-colors hover:bg-ink/10 hover:text-ink/70"
                >
              ...
              {(d.status === 'error' || d.status === 'canceled') && (
                <button
                  ...
                  title="重试"
                  className="shrink-0 rounded p-0.5 text-ink/35 transition-colors hover:bg-ink/10 hover:text-brass"
                >
