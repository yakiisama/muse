...
export default function SearchView(): React.JSX.Element {
  ...
  return (
    <div className="flex h-full flex-col px-8 pt-10">
      <div className="panel-2 flex items-center gap-2 rounded-lg px-3.5 py-2.5 ring-1 ring-ink/8 transition-shadow focus-within:ring-2 focus-within:ring-accent/60">
      ...
      <div className="mt-6 flex-1 overflow-y-auto pb-6">
        {results.length === 0 && !loading && (
          <p className="mt-16 text-center text-sm text-ink/40">输入关键词并回车开始搜索</p>
        )}
        <ul className="flex flex-col">
          {results.map((r) => {
            ...
            return (
              <li
                key={r.id}
                className="hairline-b group flex items-center gap-3 px-2.5 py-2.5 transition-colors hover:bg-ink/[0.04]"
              >
                <button
                  onClick={() => (isPreviewing ? togglePlay() : playPreview(r))}
                  disabled={previewLoading}
                  className="relative size-11 shrink-0 overflow-hidden rounded-md ring-1 ring-ink/8"
                >
                  {r.thumbnail ? (
                ...
                <div className="min-w-0 flex-1">
                  <p
                    className={`truncate text-sm ${isPreviewing ? 'text-accent font-medium' : 'text-ink/90'}`}
                  >
                    {r.title}
                  </p>
                ...
                {dl.kind === 'done' ? (
                  <span className="ml-2 flex items-center gap-1 rounded-md px-3 py-1.5 text-xs text-ink/45">
                    <Check className="size-3.5" />
                    已下载
                  </span>
                ) : dl.kind === 'downloading' ? (
                  <span className="ml-2 flex items-center gap-1 rounded-md px-3 py-1.5 text-xs text-accent">
                  ...
                  <button
                    ...
                    className={`ml-2 flex items-center gap-1 rounded-md border px-3 py-1.5 text-xs transition-colors ${
                      dl.kind === 'retry'
                        ...
                        : 'border-ink/15 text-ink/70 opacity-0 hover:border-accent hover:text-accent group-hover:opacity-100 focus-visible:opacity-100'
                    }`}
                  >
